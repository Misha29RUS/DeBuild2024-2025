...
export const Services = () => {
    ...
    const [typeService, setTypeService] = useState<'MINUTES' | 'GIGABYTES' | 'SMS'>('GIGABYTES')
    const [oneTimeService, setOneTimeService] = useState<typesServices | undefined>();
        ...
        <div className="grow px-[90px] py-10 overflow-y-auto">
            ...
            : 'opacity-0 max-h-0 overflow-hidden translate-y-4'}`}>
                <div className="flex gap-5 mb-2.5">
                    ...
                    <div className="w-[335px]">
                        ...
                        labelKey="formatName" value={oneTimeService}
            ...
            <div className="mb-10 flex items-center">
                <div className="mr-auto">
                    <Tab text="Гигабайты" select={typeService === 'GIGABYTES'}
                    onClick={() => setTypeService('GIGABYTES')}
                    styles="text-[26px] font-medium mr-5" />
                    <Tab text="Минуты" select={typeService === 'MINUTES'}
                    onClick={() => setTypeService('MINUTES')}
                    styles="text-[26px] font-medium mr-5" />
                    <Tab text="СМС" select={typeService === 'SMS'}
                    onClick={() => setTypeService('SMS')}
                    styles="text-[26px] font-medium" />
                </div>
                <Button onClick={() => setIsSidebarNewService(true)}
                    text="Новая услуга" type="red" iconLeft={<AddSvg />} />
            </div>
            <div className="grid grid-cols-3 gap-12">
                {// @ts-ignore
                    servicesData?.content.map((service, index) => (
                        <ServiceCard// @ts-ignore
                            key={index}
                            onClick={() => {
            ...
            {isSidebarOpen && <ServicesSidebar serviceID={serviceId!}
                onClose={() => {
            ...
            {isSidebarNewService && <NewServiceSidebar
               onClose={() => {
