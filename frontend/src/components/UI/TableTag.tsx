...
export const TableTag = ({ type, text, styles }: TableTagProps) => {
  const iconMap: { [key: string]: JSX.Element } = {
    ...
    DELETED: <ArchiveTariffSvg className="fill-s-white" />,
  ...
  return (
    <div
      className={`text-[18px] rounded-[20px] 
        flex items-center whitespace-nowrap truncate
         ${type === "ACTIVE" || type === "HIDDEN" || type === "DELETED" ? "py-[4px] px-[6px]" : "py-[3px] px-[6px]"}
        ${
          type === "ACTIVE"
            ? "text-s-white bg-s-red"
            : (type === "HIDDEN" || type === "DELETED")
              ? "text-s-white bg-s-black"
              : type === "GIGABYTES"
                ? "text-s-blue border border-s-blue"
                : type === "MINUTES"
                  ? "text-s-green border border-s-green"
                  : type === "SMS"
                    ? "text-s-violet border border-s-violet"
