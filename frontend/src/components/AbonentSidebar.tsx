...
export const AbonentSidebar = ({
  ...
  const renderTabContent = () => {
    switch (tab) {
      ...
      case "tariffInfo":
        return tariffInfo ? (
          <TariffInfo data={tariffInfo} services={servicesInfo!} />
        ) : null;
      case "balanceInfo":
  ...
  <div>
    <ul className="mt-5 font-normal text-[18px] user-info-ul text-s-black">
      <li>
        ...
        <p className="ml-[6px] font-light">{data.user.surname}</p>
      </li>
      <li>
        <p className="font-normal">Имя:</p>
        <p className="ml-[6px] font-light">{data.user.name}</p>
      </li>
      <li>
        <p className="text-s-black font-normal">Отчество:</p>
        <p className="ml-[6px] font-light">{data.user.patronymic}</p>
      </li>
      <li>
        <p className="text-s-black font-normal">Дата рождения:</p>
        <p className="ml-[6px] font-light">
          {`${data.user.userPassport.dateOfBirth.split("-")[2]}.${data.user.userPassport.dateOfBirth.split("-")[1]}.${data.user.userPassport.dateOfBirth.split("-")[0]}`}
        </p>
      </li>
      <li>
