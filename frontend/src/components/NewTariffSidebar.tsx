...
export const NewTariffSidebar = ({
    ...
    const [valuesGigabytes, setValuesGigabytes] = useState<(number | "min" | "max")[]>([]);
    const [defaultValuesGigabytes, setDefaultValuesGigabytes] = useState<(number | "min" | "max")[]>([minGigabytes, maxGigabytes]);
    useEffect(() => {
    ...
    const [valuesMinutes, setValuesMinutes] = useState<(number | "min" | "max")[]>([]);
    const [defaultValuesMinutes, setDefaultValuesMinutes] = useState<(number | "min" | "max")[]>([minMinutes, maxMinutes]);
    useEffect(() => {
    ...
    const [defaultValuesSms, setDefaultValuesSms] = useState<(number | "min" | "max")[]>([minSms, maxSms]);
    useEffect(() => {
    ...
    const handleSave = async () => {
        try {
            const updatedData = {
                ...
                cost: Number(cost),
                tariffResourceDto: {
                    countMinutes: Number(countMinutes) === 0 ? null : Number(countMinutes),
                    costOneMinute: Number(costMinute),
                    stepsMinutes: valuesMinutes.length > 0 ? [...valuesMinutes].sort((a, b) => a - b) : null,
                    countSms: Number(countSms) === 0 ? null : Number(countSms),
                    costOneSms: Number(costMinute),
                    stepsSms: valuesSms.length > 0 ? [...valuesSms].sort((a, b) => a - b) : null,
                    countGigabytes: Number(countGigabytes) ? null : Number(countGigabytes),
                    costOneGigabyte: Number(costMinute),
                    stepsGigabytes: valuesGigabytes.length > 0 ? [...valuesGigabytes].sort((a, b) => a - b) : null,
                },
            };
                ...
                <div className="flex-grow overflow-y-auto overflow-x-hidden">
                    <div className="flex gap-[26px] mt-5">
                        <Button onClick={handleSave}
                            type='red'  iconLeft={<Save />} text="Создать"
                            disabled={isButtonDisabled(typeTariff?.type === "CUSTOMIZABLE")}/>
                        <Button type='grey' onClick={onClose}
