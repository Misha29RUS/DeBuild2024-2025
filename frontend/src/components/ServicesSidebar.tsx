...
import {useDeleteServiceMutation, useGetServiceInfoQuery, useUpdateServiceMutation} from "../app/services/services.ts";
            ...
            <div className="flex flex-col h-[calc(100vh-80px)] p-[30px]">
                ...
                <div className="flex-grow overflow-y-auto">
                    {isAdmin && (
                        <div className="flex gap-[26px] mt-5">
                            {isEdit ? (
                                <>
                                    <Button type='red' onClick={handleSave} iconLeft={<Save />} text="Сохранить"
                                    disabled={!name || !description || !cost || (String(cost).includes('.') && !String(cost).split('.')[1]) ||
                                        (name === serviceInfo?.name && description === serviceInfo?.description && cost === serviceInfo?.cost)}/>
                ...
                {!isEdit && (
                    <div className="pt-5 border-t-[1px] border-s-light-grey">
                        <p className="text-[26px] font-light">Цена услуги: {serviceInfo?.cost} ₽</p>
                    </div>
                )}
            </div>
        </div>
